...
//我们自定义的图片验证码过滤器，把其加到SpringSecurity的过滤器链上
public class ValidateCodeFilter extends OncePerRequestFilter implements InitializingBean{
	...
	public void afterPropertiesSet() throws ServletException {
		...
		String[] configUrls =StringUtils.splitByWholeSeparatorPreserveAllTokens(securityProperties.getImage().getUrl(), ",");
	...
	private void myValidate(ServletWebRequest request) throws ServletRequestBindingException {
		...
		if (codeInSession.isExpried()) {
			sessionStrategy.removeAttribute(request, ValidateCodeController.SESSION_KEY);	//过期了就把验证码从session中移除掉
